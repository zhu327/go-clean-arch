...
Skills (`.agents/skills/`, shared by pi and Cursor): `grill-me` (focused design interview for consequential decisions), `plan-execute` (parallel/high-risk planning + wave execution), `code-review-expert` (changeset review methodology), `improve-codebase-architecture` (deepening assessment). Invocation policy: `improve-codebase-architecture` is human-triggered only (`disable-model-invocation: true`); the other skills remain model-invocable.

Agents (`.pi/agents/`, `.cursor/agents/`): `code-reviewer` (read-only independent review), `code-simplifier` (targeted behavior-preserving simplification). At most one full changeset review; re-review fixes only when they reshape the design.
