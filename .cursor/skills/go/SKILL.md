...
- Inspect repository rules, code, tests, and CI before choosing commands or architecture. Use kfinops conventions under `.cursor/rules/` and never invent tooling.
...
| **High-risk** | Auth/tenancy, migrations, destructive writes, concurrency, billing, privacy, or hard-to-reverse public contracts | Use AskQuestion to agree design and verification/rollback; require independent review |
...
Use `code-reviewer` when security, authorization, tenancy, billing, migration, concurrency, destructive behavior, public contracts, or cross-domain architecture changed; when multiple implementers contributed; when the diff is difficult to reason about; or when the user asks.
